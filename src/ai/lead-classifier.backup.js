...
module.exports = LeadClassifier;
  ...
  // Gerar perguntas qualificadoras baseadas no tipo
  gerarPerguntasQualificadoras(classificacao, dadosColetados) {
    const perguntas = [];

    if (classificacao.tipo === 'COMPRADOR') {
      // Perguntas para leads novos
      if (!dadosColetados.dispositivo) {
        perguntas.push("Em qual dispositivo você gostaria de usar? (TV, celular, Fire Stick)");
      }
      
      if (!dadosColetados.intencao) {
        perguntas.push("Gostaria de fazer um teste gratuito primeiro?");
      }
      
      perguntas.push("Já conhece nossos planos? Posso apresentar as opções?");
      
    } else if (classificacao.tipo === 'CLIENTE') {
      // Perguntas para clientes existentes
      if (!dadosColetados.problema && !dadosColetados.aplicativo) {
        perguntas.push("Qual problema específico está enfrentando?");
        perguntas.push("Qual aplicativo você está usando?");
      }
      
      if (!dadosColetados.dispositivo) {
        perguntas.push("Em qual dispositivo está tentando usar?");
      }
      
      perguntas.push("Quando foi a última vez que funcionou normalmente?");
      
    } else {
      // Perguntas para indefinidos
      perguntas.push("Você já é nosso cliente ou está conhecendo agora?");
      perguntas.push("Posso ajudar com algum problema ou tem interesse em nossos planos?");
    }

    return perguntas.slice(0, 2); // Máximo 2 perguntas por vez
  }

  // Análise contextual completa
  analisarContexto(mensagem, contato, historicoCompleto = []) {
    const classificacao = this.classificarLead(mensagem, historicoCompleto);
    const dadosColetados = this.extrairDados(mensagem);
    const perguntas = this.gerarPerguntasQualificadoras(classificacao, dadosColetados);
    ...
    return {
      ...
      contexto_completo: {
        é_novo_contato: historicoCompleto.length === 0,
        total_interacoes: historicoCompleto.length,
        ultima_interacao: historicoCompleto.length > 0 ? 
          historicoCompleto[historicoCompleto.length - 1].timestamp : null,
        tags_sugeridas: this.sugerirTags(classificacao, dadosColetados)
      }
    };
  }

  // Sugerir tags baseadas na análise
  sugerirTags(classificacao, dadosColetados) {
    const tags = [];
    ...
    if (classificacao.tipo === 'COMPRADOR') {
      ...
      if (classificacao.confianca > 70) {
        tags.push('Alta Conversão');
      }
    } else if (classificacao.tipo === 'CLIENTE') {
      tags.push('Cliente Ativo');
      if (dadosColetados.problema) {
        tags.push('Suporte Técnico');
      }
    }
    ...
    // Tags por dados coletados
    if (dadosColetados.dispositivo) {
      tags.push(`Dispositivo: ${dadosColetados.dispositivo}`);
    }
    
    if (dadosColetados.aplicativo) {
      tags.push(`App: ${dadosColetados.aplicativo}`);
    }
